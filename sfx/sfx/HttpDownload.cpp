...
bool CHttpDownloader::DownloadHttpFile(const TCHAR *szUrl, const TCHAR *szDestFile, const TCHAR *szDestDir, HANDLE hEvtAbort, DOWNLOAD_STATUS_CALLBACK *usercb)
{
	if (!szUrl)
		return false;
                ...
                if (HttpQueryInfo(m_hUrl, HTTP_QUERY_CONTENT_LENGTH, buf_query, &buflen, NULL) || chunked)
                {
					...
					if (hfile != INVALID_HANDLE_VALUE)
					{
						...
						do
						{
							...
							DWORD amount_to_write = rxall ? 0 : inbuf[bufidx].dwBufferLength;

							// report to the user how much we've received and how much we expect to get overall
							if (usercb)
								usercb(amount_written, amount_to_download);

							rxall = (amount_received >= amount_to_download);
							...
							inbuf[bufidx].dwBufferLength = std::min<DWORD>(BUFFER_SIZE, (DWORD)std::min<uint64_t>(MAXDWORD, (amount_to_download - amount_received)));

							// if we have more left to read, then do that...
							if (!rxall && inbuf[bufidx].dwBufferLength)
							{
						...
						while (amount_written < amount_to_download);

						CloseHandle(data_written_event);

						// we should have the file now
						retval = (amount_written == amount_to_download);
