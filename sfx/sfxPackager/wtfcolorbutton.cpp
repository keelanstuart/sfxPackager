...
IMPLEMENT_DYNAMIC(CWTFColorButton, CMFCButton)

CWTFColorButton::CWTFColorButton()
{
	m_Color = RGB(0, 0, 0);
...
BEGIN_MESSAGE_MAP(CWTFColorButton, CMFCButton)
	ON_WM_KEYDOWN()
	ON_WM_LBUTTONDOWN()
	ON_WM_GETDLGCODE()
	ON_WM_SYSCOLORCHANGE()
	ON_WM_MOUSEMOVE()
	ON_MESSAGE(WM_MFC_INITCTRL, &CWTFColorButton::OnInitControl)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CWTFColorButton message handlers

CSize CWTFColorButton::SizeToContent(BOOL bCalcOnly)
{
...
void CWTFColorButton::OnFillBackground(CDC* pDC, const CRect& rectClient)
{
	if (!IsDrawXPTheme())
	{
...
void CWTFColorButton::OnDraw(CDC* pDC, const CRect& rect, UINT uiState)
{
	ASSERT_VALID(pDC);
...
void CWTFColorButton::OnDrawBorder(CDC* pDC, CRect& rectClient, UINT /*uiState*/)
{
	ASSERT_VALID(pDC);
...
void CWTFColorButton::OnDrawFocusRect(CDC* pDC, const CRect& rectClient)
{
	CSize sizeArrow = CMenuImages::Size();
...
void CWTFColorButton::OnShowColorPopup()
{
	if (m_pPopup != NULL)
	{
	...
	{
		// Use default pallete:
		CWTFColorBar::InitColors(NULL, m_Colors);
	}

	m_pPopup = new CWTFColorPopupMenu(this, m_Colors, m_Color, m_strAutoColorText, m_strOtherText, m_strDocColorsText, m_lstDocColors, m_nColumns, m_ColorAutomatic);
	...
	if (!m_pPopup->Create(this, rectWindow.left, rectWindow.bottom, NULL, m_bEnabledInCustomizeMode))
	{
		...
		TRACE(_T("Color menu can't be used in the customization mode. You need to set CWTFColorButton::m_bEnabledInCustomizeMode\n"));
	}
	else
	{
		if (m_bEnabledInCustomizeMode)
		{
			CWTFColorBar* pColorBar = DYNAMIC_DOWNCAST(CWTFColorBar, m_pPopup->GetMenuBar());
...
void CWTFColorButton::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CWTFColorButton::OnLButtonDown(UINT /*nFlags*/, CPoint /*point*/)
{
	SetFocus();
	OnShowColorPopup();
}

void CWTFColorButton::OnMouseMove(UINT nFlags, CPoint point)
{
...
UINT CWTFColorButton::OnGetDlgCode()
{
	return DLGC_WANTARROWS;
}

void CWTFColorButton::EnableAutomaticButton(LPCTSTR lpszLabel, COLORREF colorAutomatic, BOOL bEnable)
{
	m_strAutoColorText = (bEnable && lpszLabel == NULL) ? _T("") : lpszLabel;
	m_ColorAutomatic = colorAutomatic;
}

void CWTFColorButton::EnableOtherButton(LPCTSTR lpszLabel, BOOL bAltColorDlg, BOOL bEnable)
{
	m_strOtherText = (bEnable && lpszLabel == NULL) ? _T("") : lpszLabel;
	m_bAltColorDlg = bAltColorDlg;
}

void CWTFColorButton::SetDocumentColors(LPCTSTR lpszLabel, CList<COLORREF,COLORREF>& lstColors)
{
...
void CWTFColorButton::SetPalette(CPalette* pPalette)
{
	if (m_Colors.GetSize() != 0)
	{
		m_Colors.SetSize(0);
...
void CWTFColorButton::SetColor(COLORREF color /* -1 - automatic*/)
{
	m_Color = color;
...
void CWTFColorButton::UpdateColor(COLORREF color)
{
	SetColor(color);
...
void CWTFColorButton::RebuildPalette(CPalette* pPal)
{
	if (m_pPalette != NULL)
	{
		delete m_pPalette;
...
void CWTFColorButton::OnSysColorChange()
{
...
BOOL CWTFColorButton::IsDrawXPTheme() const
{
	return m_bWinXPTheme && CMFCVisualManager::GetInstance()->IsWindowsThemingSupported();
}

LRESULT CWTFColorButton::OnInitControl(WPARAM wParam, LPARAM lParam)
{
