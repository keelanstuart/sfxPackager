...
CMap<UINT,UINT,COLORREF, COLORREF> CWTFColorMenuButton::m_ColorsByID;

UINT WTF_WM_GETDOCUMENTCOLORS = ::RegisterWindowMessage(_T("TOOLBAR__GETDOCUMENTCOLORS"));

IMPLEMENT_SERIAL(CWTFColorMenuButton, CMFCToolBarMenuButton, VERSIONABLE_SCHEMA | 1)

// Construction/Destruction
CWTFColorMenuButton::CWTFColorMenuButton()
{
	Initialize();
}

CWTFColorMenuButton::CWTFColorMenuButton(UINT uiCmdID, LPCTSTR lpszText, CPalette* pPalette) :
...
void CWTFColorMenuButton::Initialize()
{
...
void CWTFColorMenuButton::EnableAutomaticButton(LPCTSTR lpszLabel, COLORREF colorAutomatic, BOOL bEnable)
{
...
void CWTFColorMenuButton::EnableOtherButton(LPCTSTR lpszLabel, BOOL bAltColorDlg, BOOL bEnable)
{
...
void CWTFColorMenuButton::EnableDocumentColors(LPCTSTR lpszLabel, BOOL bEnable)
{
...
void CWTFColorMenuButton::EnableTearOff(UINT uiID, int nVertDockColumns, int nHorzDockRows)
{
...
void CWTFColorMenuButton::OnDraw(CDC* pDC, const CRect& rect, CMFCToolBarImages* pImages, BOOL bHorz, BOOL bCustomizeMode, BOOL bHighlight, BOOL bDrawBorder, BOOL bGrayDisabledButtons)
{
	ASSERT_VALID(this);
	ASSERT_VALID(pDC);
	...
	if (GetGlobalData()->m_nBitsPerPixel == 8) // 256 colors
	{
		if (m_Palette.GetSafeHandle() == NULL)
		{
			...
			CWTFColorBar::CreatePalette(m_Colors, m_Palette);
		}
...
int CWTFColorMenuButton::OnDrawOnCustomizeList(CDC* pDC, const CRect& rect, BOOL bSelected)
{
	int nID = m_nID;
...
void CWTFColorMenuButton::SetColor(COLORREF clr, BOOL bNotify)
{
	m_Color = clr;
	...
	if (bNotify)
	{
		...
		if (CMFCToolBar::GetCommandButtons(m_nID, listButtons) > 0)
		{
			...
			{
				CWTFColorMenuButton* pOther = DYNAMIC_DOWNCAST(CWTFColorMenuButton, listButtons.GetNext(pos));
		...
		{
			CWTFColorBar* pColorBar = DYNAMIC_DOWNCAST(CWTFColorBar, lstToolBars.GetNext(pos));
...
void CWTFColorMenuButton::OnChangeParentWnd(CWnd* pWndParent)
{
...
void CWTFColorMenuButton::Serialize(CArchive& ar)
{
...
void CWTFColorMenuButton::CopyFrom(const CMFCToolBarButton& s)
{
	CMFCToolBarMenuButton::CopyFrom(s);

	const CWTFColorMenuButton& src = (const CWTFColorMenuButton&) s;
...
BOOL CWTFColorMenuButton::OpenColorDialog(const COLORREF colorDefault, COLORREF& colorRes)
{
	BOOL bResult = FALSE;
...
CMFCPopupMenu* CWTFColorMenuButton::CreatePopupMenu()
{
	...
	if (m_bIsDocumentColors && m_pWndParent != NULL)
	{
		...
		pOwner->SendMessage(WTF_WM_GETDOCUMENTCOLORS, (WPARAM) m_nID, (LPARAM) &lstDocColors);
	}
...
void __stdcall CWTFColorMenuButton::SetColorName(COLORREF color, const CString& strName)
{
	CWTFColorBar::m_ColorNames.SetAt(color, strName);
}

COLORREF __stdcall CWTFColorMenuButton::GetColorByCmdID(UINT uiCmdID)
{
