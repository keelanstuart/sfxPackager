...
/*============================================================================*/
// CWTFPropertyGridProperty object

class CWTFPropertyGridProperty : public CObject
{
	DECLARE_DYNAMIC(CWTFPropertyGridProperty)

	friend class CWTFPropertyGridCtrl;

// Construction
public:
	// Group constructor
	CWTFPropertyGridProperty(const CString& strGroupName, DWORD_PTR dwData = 0, BOOL bIsValueList = FALSE);

	// Simple property
	CWTFPropertyGridProperty(const CString& strName, const COleVariant& varValue, LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0,
		LPCTSTR lpszEditMask = NULL, LPCTSTR lpszEditTemplate = NULL, LPCTSTR lpszValidChars = NULL);

	virtual ~CWTFPropertyGridProperty();
...
public:
	...
	BOOL AddSubItem(CWTFPropertyGridProperty* pProp);
	BOOL RemoveSubItem(CWTFPropertyGridProperty*& pProp, BOOL bDelete = TRUE);
	...
	CWTFPropertyGridProperty* HitTest(CPoint point, CWTFPropertyGridProperty::ClickArea* pnArea = NULL);
...
protected:
	void Init();
	void SetFlags();
	void SetOwnerList(CWTFPropertyGridCtrl* pWndList);
	void Reposition(int& y);
	void AddTerminalProp(CList<CWTFPropertyGridProperty*, CWTFPropertyGridProperty*>& lstProps);

	BOOL IsSubItem(CWTFPropertyGridProperty* pProp) const;
	CWTFPropertyGridProperty* FindSubItemByData(DWORD_PTR dwData) const;
...
public:
	...
	virtual void OnSetSelection(CWTFPropertyGridProperty* /*pOldSel*/) {}
	virtual void OnKillSelection(CWTFPropertyGridProperty* /*pNewSel*/) {}
...
//	virtual CWTFColorSlidersPopupMenu* CreateColorSliderEditor(CWnd* pWndParent, CRect rect);
...
public:
	...
	CWTFPropertyGridProperty* GetSubItem(int nIndex) const;

	CWTFPropertyGridProperty* GetParent() const { return m_pParent; }
...
protected:
	...
	CWTFPropertyGridCtrl*     m_pWndList;    // Pointer to the PropertyList window
	CWTFPropertyGridProperty* m_pParent;     // Parent property (NULL for top-level properties)

	CList<CWTFPropertyGridProperty*, CWTFPropertyGridProperty*> m_lstSubItems; // Sub-properies list
};

/*============================================================================*/
// CWTFPropertyGridColorProperty object

class CWTFPropertyGridColorProperty : public CWTFPropertyGridProperty
{
	friend class CWTFPropertyGridCtrl;

	DECLARE_DYNAMIC(CWTFPropertyGridColorProperty)

// Construction
public:
	CWTFPropertyGridColorProperty(const CString& strName, const COLORREF& color, CPalette* pPalette = NULL, LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0);
	virtual ~CWTFPropertyGridColorProperty();
...
class CWTFPropertyGridFileProperty : public CWTFPropertyGridProperty
{
	DECLARE_DYNAMIC(CWTFPropertyGridFileProperty)

// Construction
public:

	CWTFPropertyGridFileProperty(const CString& strName, const CString& strFolderName, DWORD_PTR dwData = 0, LPCTSTR lpszDescr = NULL);
	CWTFPropertyGridFileProperty(const CString& strName, BOOL bOpenFileDialog, const CString& strFileName, LPCTSTR lpszDefExt = NULL,
		DWORD dwFlags = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, LPCTSTR lpszFilter = NULL, LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0);
	virtual ~CWTFPropertyGridFileProperty();
...
class CWTFPropertyGridFontProperty : public CWTFPropertyGridProperty
{
	DECLARE_DYNAMIC(CWTFPropertyGridFontProperty)

// Construction
public:
	CWTFPropertyGridFontProperty(	const CString& strName, LOGFONT& lf, DWORD dwFontDialogFlags = CF_EFFECTS | CF_SCREENFONTS, 
		LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0, COLORREF color = (COLORREF)-1);
	virtual ~CWTFPropertyGridFontProperty();
...
/*============================================================================*/
// CWTFPropertyGridCtrl window

#define AFX_PROPLIST_ID_INPLACE 3


class CWTFPropertyGridCtrl : public CWnd
{
	DECLARE_DYNAMIC(CWTFPropertyGridCtrl)

	friend class CWTFPropertyGridProperty;
...
public:
	...
	static UINT AFX_WM_PROPERTY_CHANGED;
...
protected:
	...
	CWTFPropertyGridToolTipCtrl m_IPToolTip;     // Inplace tooltip control
	...
	int m_nRowVerticalPadding; // Added to m_nRowHeight
	int m_nSubItemIndent;      // How much sub-items are indented by on the left
	...
	CList<CWTFPropertyGridProperty*, CWTFPropertyGridProperty*> m_lstProps;         // List of top-level properties
	CList<CWTFPropertyGridProperty*, CWTFPropertyGridProperty*> m_lstTerminalProps; // List of terminal properties
	CWTFPropertyGridProperty*                            m_pSel;             // Current selection
...
// Operations
public:
	int AddProperty(CWTFPropertyGridProperty* pProp, BOOL bRedraw = TRUE, BOOL bAdjustLayout = TRUE);
	BOOL DeleteProperty(CWTFPropertyGridProperty*& pProp, BOOL bRedraw = TRUE, BOOL bAdjustLayout = TRUE);
	void RemoveAll();

	CWTFPropertyGridProperty* GetProperty(int nIndex) const;
	int GetPropertyCount() const { return (int) m_lstProps.GetCount(); }

	CWTFPropertyGridProperty* FindItemByData(DWORD_PTR dwData, BOOL bSearchSubItems = TRUE) const;

	CWTFPropertyGridProperty* HitTest(CPoint pt, CWTFPropertyGridProperty::ClickArea* pnArea = NULL, BOOL bPropsOnly = FALSE) const;

	void SetCurSel(CWTFPropertyGridProperty* pProp, BOOL bRedraw = TRUE);
	CWTFPropertyGridProperty* GetCurSel() const { return m_pSel; }

	void EnsureVisible(CWTFPropertyGridProperty* pProp, BOOL bExpandParents = FALSE);
...
public:
	virtual void OnChangeSelection(CWTFPropertyGridProperty* /*pNewSel*/, CWTFPropertyGridProperty* /*pOldSel*/) {}

	virtual BOOL EditItem(CWTFPropertyGridProperty* pProp, LPPOINT lptClick = NULL);
	virtual void OnClickButton(CPoint point);
	virtual BOOL EndEditItem(BOOL bUpdateData = TRUE);
	virtual BOOL ValidateItemData(CWTFPropertyGridProperty* /*pProp*/) { return TRUE; }

	virtual int OnDrawProperty(CDC* pDC, CWTFPropertyGridProperty* pProp) const;
...
protected:
	...
	virtual int CompareProps(const CWTFPropertyGridProperty* pProp1, const CWTFPropertyGridProperty* pProp2) const;
	virtual void NotifyAccessibility(CWTFPropertyGridProperty* pProp);

// Implementation
public:
	virtual ~CWTFPropertyGridCtrl();

	//----------------------
	// Accessibility support:
	//----------------------
	CWTFPropertyGridProperty* m_pAccProp;
